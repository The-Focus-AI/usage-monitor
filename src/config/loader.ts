...
export class ConfigLoader {
	...
	constructor(configPath?: string) {
		this.configPath =
			configPath || resolve(process.cwd(), "usage-monitor.json");
	}
	...
	async load(): Promise<Config> {
		...
		if (existsSync(this.configPath)) {
			try {
				...
				console.warn(
					`Warning: Failed to load config from ${this.configPath}:`,
					error,
				);
			}
		}
	...
	private mergeWithEnvironment(fileConfig: any): any {
		const envConfig: any = {
			...
			notifications: {},
			thresholds: {
				alertThresholdUsd: safeParseFloat(
					process.env.BALANCE_ALERT_THRESHOLD_USD,
					10,
				),
		...
		const openrouterApiKey =
			process.env.OPENROUTER_API_KEY || process.env.OPENROUTER_API_TOKEN;
		if (openrouterApiKey) {
