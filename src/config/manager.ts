...
import { ServicesConfig, ServicesConfigSchema, type ServiceConfig } from './services.js';
...
export class ConfigManager {
  ...
  constructor(configPath?: string) {
    this.configPath = configPath || resolve(process.cwd(), 'usage-monitor-config.json');
  ...
  async save(config: ServicesConfig): Promise<void> {
    try {
      ...
      throw new Error(`Failed to save config to ${this.configPath}: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  async getCredential(serviceId: string): Promise<string> {
    ...
    // Try 1Password first
    if (serviceConfig.credentialSource === '1password' || serviceConfig.credentialSource === 'both') {
      if (serviceConfig.onePasswordRef) {
        try {
          const credential = await this.opClient.readReference(serviceConfig.onePasswordRef);
          if (credential) return credential;
        } catch (error) {
          console.warn(`1Password lookup failed for ${serviceId}: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
      }
    }

    // Fallback to environment variable
    if (serviceConfig.credentialSource === 'env' || serviceConfig.credentialSource === 'both') {
   ...
   */
  async addService(serviceId: string, config: Partial<ServiceConfig>): Promise<void> {
    const currentConfig = await this.load();
    
    currentConfig.services[serviceId] = {
      enabled: true,
      credentialSource: '1password',
      fallbackEnvVar: config.fallbackEnvVar || `${serviceId.toUpperCase()}_API_KEY`,
      ...config,
    };
   ...
   */
  async testServices(): Promise<Record<string, { success: boolean; error?: string }>> {
    const config = await this.load();
    ...
    for (const [serviceId, serviceConfig] of Object.entries(config.services)) {
      ...
      try {
        const credential = await this.getCredential(serviceId);
