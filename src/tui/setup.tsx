...
interface AppState {
	...
	selectedItem: OnePasswordItem | undefined;
...
function SetupApp() {
	...
	async function testAndSaveService() {
		...
		try {
			...
			try {
				apiKey = await opClient.getItemField(
			...
			await state.configManager.addService(state.selectedServiceType, {
				...
				onePasswordRef,
				fallbackEnvVar:
					KNOWN_SERVICES[state.selectedServiceType].fallbackEnvVar,
				description,
	...
	if (state.stage === "loading-all-keys") {
		return (
			<Box flexDirection="column" padding={1}>
				<Text color="blue">
					🔍 Loading all API keys from 1Password vaults...
				</Text>
				<Newline />
				<Text color="gray">
					Searching across {state.vaults.length} vaults for potential API
					keys...
				</Text>
			</Box>
		);
	...
	if (state.stage === "key-selection") {
		const displayItems =
			state.filteredApiKeys.length > 0
				? state.filteredApiKeys
				: state.allApiKeys;
		...
		if (displayItems.length === 0) {
			return (
				<Box flexDirection="column" padding={1}>
					<Text color="yellow">
						❓ No API keys found in any 1Password vault.
					</Text>
					<Newline />
					<Text color="gray">
						Make sure your API keys have titles containing service names like:
					</Text>
					<Text color="gray">• "OpenRouter API Key"</Text>
					<Text color="gray">• "OpenAI Key"</Text>
					<Text color="gray">• "Claude API"</Text>
					<Text color="gray">
						• Or any item with "key", "token", or "api" in the title
					</Text>
				</Box>
			);
		}
		...
		const keyItems = displayItems.map((item, index) => {
			const guessedType = guessServiceType(item.title);
			const serviceName = guessedType
				? ` → ${KNOWN_SERVICES[guessedType].name}`
				: " → Manual setup";
		...
		return (
			<Box flexDirection="column" padding={1}>
				<Text color="green" bold>
					🔑 Found {state.allApiKeys.length} API keys across all vaults
				</Text>
				<Newline />
				...
				<Text color="gray">
					Showing {displayItems.length} of {state.allApiKeys.length} keys:
				</Text>
				<Newline />
	...
	if (state.stage === "service-selection") {
		...
		return (
			<Box flexDirection="column" padding={1}>
				<Text color="green" bold>
					⚙️ Select service type for '{state.selectedItem?.title}'
				</Text>
				<Newline />

				<Text color="gray">
					Could not auto-detect service type. Please select manually:
				</Text>
				<Newline />
	...
	if (state.stage === "api-testing") {
		return (
			<Box flexDirection="column" padding={1}>
				<Text color="blue">
					🧪 Testing '{state.selectedItem?.title}' as{" "}
				...
				{state.selectedServiceType && (
					<Box flexDirection="column">
						<Text color="green" bold>
							{KNOWN_SERVICES[state.selectedServiceType].name}
						</Text>
						<Text color="gray">
							{KNOWN_SERVICES[state.selectedServiceType].description}
						</Text>
						<Newline />

						<Text color="blue">📍 1Password Reference:</Text>
						<Text color="gray">
							op://{state.selectedItem?.vault.name}/{state.selectedItem?.title}
							/[auto-detected field]
						</Text>
						<Newline />
				...
				<Text color="gray">
					•{" "}
					{state.selectedServiceType === "openrouter"
						? "Testing API connection and fetching usage data"
						: "Saving configuration (testing not implemented yet)"}
				</Text>
				<Text color="gray">
					• Saving configuration to usage-monitor-config.json
				</Text>
	...
	if (state.stage === "complete") {
		return (
			<Box flexDirection="column" padding={1}>
				...
				<Text>
					• {state.selectedServiceType} - {state.selectedItem?.title}
				</Text>
				<Text>
					• 1Password: {state.selectedItem?.vault.name}/
					{state.selectedItem?.title}
				</Text>
				<Text>
